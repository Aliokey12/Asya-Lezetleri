...
menu.forEach(function(item) {
    MenuSet.add(item.category)
})

MenuSet.forEach(function(item) {
    let btnDOM = document.createElement('span')
    btnDOM.classList.add("btn-item")
    btnDOM.style = btnStyle
    btnDOM.setAttribute('id', `${item}`)
    btnDOM.innerHTML = `${item}`
...
menu.forEach(AddList) /
    function AddList(item) {
        ...
        divDOM.setAttribute('name', item.category)
        divDOM.setAttribute('id', item.id)
        let imgDOM = document.createElement('img')
        imgDOM.src = item.img
        imgDOM.alt = item.title
